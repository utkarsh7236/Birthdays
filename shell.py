...
for i in range(len(dates)):
    ...
    center = f"{dates[i][0].days} days"
    right_aligned = f"({dates[i][2]})"

    if dates[i][0].days == 1:
        center = f"{dates[i][0].days} day"

    elif dates[i][0].days == 0:
        center = "Today"

    if dates[i][0].days <= 5:
        print(color.BOLD + color.RED + f"{left_aligned:<25}{center:^10}{right_aligned:>10}" + color.END)

    elif 5 < dates[i][0].days <= 15:
        print(color.BOLD + color.YELLOW + f"{left_aligned:<25}{center:^10}{right_aligned:>10}" + color.END)

    elif 15 < dates[i][0].days <= 30:
        print(color.BOLD + color.GREEN + f"{left_aligned:<25}{center:^10}{right_aligned:>10}" + color.END)

    else:
        print(color.BOLD + color.DIM + f"{left_aligned:<25}{center:^10}{right_aligned:>10}" + color.END)
